...
- `@feature-builder` — pioyan-chat の機能を TDD サイクルで実装。Next.js （フロント）・ FastAPI （バック）両対応。PLAN.md のチェックリストを自動更新
  - 定義: [.github/agents/feature-builder.md](.github/agents/feature-builder.md)
...
- `@ui-ux-designer` — ソフトグローグラデーションを基調としたUIデザインとUX改善を担当。コンポーネントのスタイルレビュー・改善提案・実装
  - 定義: [.github/agents/ui-ux-designer.md](.github/agents/ui-ux-designer.md)
...
```text
1. PLAN.md でタスクを確認し、feature ブランチを作成（必要なら /git-workflow を参照）
2. @feature-builder で TDD サイクルに従い機能を実装
3. @code-reviewer でセルフレビュー
4. @docs-writer でドキュメント更新を確認
5. PR 作成 → CI 通過を確認
```
